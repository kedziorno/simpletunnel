...
void server_tcp::run() {
	m_acceptor.async_accept(m_socket_tcp_ssl.lowest_layer(), m_endpoint, [&](const boost::system::error_code & error_code){
		if (error_code) {
			pfp_fact("Accept error : " << error_code.message());
		} else {
		...
		m_socket_tcp_ssl.async_handshake(boost::asio::ssl::stream_base::server, [&](const boost::system::error_code & error_code){
			if (error_code) {
				pfp_fact("handshake : failed , " << error_code.message());
			} else {
